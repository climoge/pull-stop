...
public class PhysicBody extends Actor implements Moveable{
	...
	public PhysicBody(float posX, float posY, Texture texture, World world, float density, float restitution, float friction, boolean rotation) {
		...
		createBody(posX + texture.getWidth() / 2 / PIXELS_TO_METERS, posY + texture.getHeight() / 2 / PIXELS_TO_METERS, world, density, restitution, friction, rotation);
	}

	public PhysicBody(float posX, float posY, Texture texture, World world) {
		this(posX, posY, texture, world, 0.3f, 0f, 0f, true);
	}
